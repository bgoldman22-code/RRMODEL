// netlify/functions_live/hits2-slate.mjs
// Build today's MLB 2+ hits slate from odds-first list (players that books actually price) + StatsAPI model.
// Steps: get odds (player_hits Over 1.5) -> map names to MLBAM ids -> pull season/L15 stats -> compute P(2+ hits) -> EV.
...
// Map "First Last" -> MLBAM id using MLB lookup service
async function lookupMLBId(name) {
  const part = encodeURIComponent(name.split(" ").slice(-1)[0]); // use last name for broader match
  ...
  // Try exact (case-insensitive) fullName match first
  const lower = name.toLowerCase();
  const exact = rows.find(r => (r?.name_display_first_last||"").toLowerCase() === lower || (r?.name_display_last_first||"").toLowerCase() === lower);
  const best = exact || rows.find(r => (r?.name_display_first_last||"").toLowerCase().includes(lower.split(" ")[0]) && (r?.name_display_first_last||"").toLowerCase().includes(lower.split(" ").slice(-1)[0]));
...
export const handler = async (event) => {
  try {
    ...
    for (const o of offers) {
      ...
      if (id) mapNameToId.set(o.player, id);
    }
    ...
    for (const o of offers) {
      ...
      if (!st) continue; // skip players we couldn't map cleanly
    ...
    return { statusCode: 200, headers: { "content-type":"application/json" }, body: JSON.stringify({ ok:true, date, path: "oddsfirst", count: rows.length, players: rows }) };
  } catch (err) {
