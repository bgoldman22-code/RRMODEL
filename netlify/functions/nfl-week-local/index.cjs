  ...
  try {
    ...
    if (!week || week === 'auto') {
      ...
      if (!sched.weeks[String(week)]) {
        return { statusCode: 400, headers: { 'content-type': 'application/json' }, body: JSON.stringify({ ok:false, error:`Unknown week ${week}`, meta: { dataPath: dataPath(), dirname: __dirname } }) };
      }
    }
