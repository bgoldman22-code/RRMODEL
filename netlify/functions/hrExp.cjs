// src/utils/hrExp.cjs
// Single place for: lineup validation, slugger floor, cluster bump, caps.
...
const CAPS = {
  maxProb: 0.60,
  maxAdjBump: 0.03,      // cap total slugger+cluster bump
...
// Hard rule: drop known outs / IL / OUT
...
// Mid-weight: slugger floor
function applySluggerFloor(pick){
  const isSlugger = SLUGGERS.has(pick.player);
  if (!isSlugger) return { applied:false, newProb: Number(pick.model_hrp||0) };

  const base = Number(pick.model_hrp||0);
  // require modest baseline or EV flag (if you pass it as pick.ev_positive)
...
// Team cluster: if a team has >=2 strong bats (>=0.26 base), bump other eligible teammates
function computeTeamStats(picks){
  const byTeam = {};
  for (const p of picks){
    const t = p.team || p.team_abbr || "UNK";
    if (!byTeam[t]) byTeam[t] = { strong:0, list:[] };
    if ((Number(p.model_hrp)||0) >= 0.26) byTeam[t].strong++;
...
function finalizeAdjusted(picks, flagsByPlayer){
  return picks.map(p => {
    ...
    if (totalBump > CAPS.maxAdjBump) proposed = base + CAPS.maxAdjBump;
...
function buildTracks(baselinePicks, knownOut=[]) {
  ...
  // CONTROL: baseline as-is (post-validation)
  ...
  // ADJUSTED: slugger floor + cluster bump (mid-weight, capped)
...
module.exports = { buildTracks, isoDateET, CAPS };
