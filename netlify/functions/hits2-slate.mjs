...
export const handler = async (event) => {
  try {
    ...
    // normalize outcomes across potential shapes
    const pushOffer = (o) => {
      if (!o) return;
      ...
      const isRightPoint = Math.abs(point - 1.5) < 1e-9; // strict 1.5
      ...
      if (!existing || priceDec > existing.priceDec) {
        playersMap.set(key, { player, game, bookmaker: String(bookmaker || '').toLowerCase(), priceDec, priceAm, point });
      }
    };

    // Flatten any of the common TheOddsAPI shapes
    for (const g of offers) {
      if (!g) continue;

      // direct shape: market/outcomes at root
    ...
    for (const { player, game, bookmaker, priceDec, priceAm, point } of playersMap.values()) {
      const implied = 1 / priceDec; // crude implied probability (no-vig)
      players.push({
        player,
        team: '',
        game,
