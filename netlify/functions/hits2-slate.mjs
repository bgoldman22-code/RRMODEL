...
const MARKET_KEYS = ["batter_hits", "batter_hits_alternate"];
...
// Walk any nesting shape for outcomes
  ...
  if (offer?.markets?.length) {
    for (const m of offer.markets) {
      if (m?.outcomes?.length) {
        yield* m.outcomes.map(oc => ({ oc, src: "markets", bookmaker: m.bookmaker || offer.bookmaker }));
      }
    }
  }
  if (offer?.bookmakers?.length) {
    for (const bk of offer.bookmakers) {
      for (const m of bk.markets || []) {
        if (m?.outcomes?.length) {
          yield* m.outcomes.map(oc => ({ oc, src: "bookmakers", bookmaker: bk.title || bk.key || bk.name }));
        }
      }
    }
  }
...
function collectOffers(raw) {
  ...
  for (const o of (raw.offers || [])) {
    ...
    for (const { oc, bookmaker } of outcomeWalk(o)) {
      ...
      // STRICT: only Over 1.5 (i.e., 2+ in some UIs)
    ...
    // keep the best (highest decimal) per player+game for 1.5 line
    const best = new Map();
...
export const handler = async (event) => {
  try {
    ...
    const oddsUrl = `${base}/.netlify/functions/odds-hits2?date=${encodeURIComponent(date)}`;
    ...
    // map to UI rows (fallback model = odds-implied prob until model wires in)
    const players = rows.map(r => {
      const modelProb = decToProb(r.decimal);
    ...
    return jsonResponse({
      ok: true,
      date,
      meta: { provider: odds.provider, usingOddsApi: odds.usingOddsApi, market: "batter_hits(Over 1.5 only)" },
