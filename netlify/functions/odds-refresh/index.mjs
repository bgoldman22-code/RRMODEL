...
// Fetches NFL H2H moneyline odds from TheOddsAPI and caches to Blobs per week.
...
export default async (req, context) => {
  try {
    ...
    if (existing && !force) {
      return json({ ok: true, cached: true, key: `odds_week_${week}.json`, count: existing.rows?.length || 0 });
    }

    const KEY = process.env.ODDS_API_KEY;
    if (!KEY) return json({ error: 'Missing ODDS_API_KEY' }, 400);
    ...
    for (const evt of events || []) {
      ...
      rows.push({
        home, away, ml_home, ml_away,
        event_id: evt.id,
        commence_time: evt.commence_time
      });
    }

    const data = { week, updatedAt: new Date().toISOString(), meta: { source: 'theoddsapi', bookmaker }, rows };
    await nflBlobsPutJSON(`odds_week_${week}.json`, data);

    return json({ ok: true, wrote: rows.length, key: `odds_week_${week}.json`, meta: data.meta });
  } catch (err) {
