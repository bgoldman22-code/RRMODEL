...
export class PlayerWeekAvailability {
  ...
  async _calculateSkillPositionImpact() {
    ...
    // V2 HAD ENHANCEMENT: Use trustedDepth from HAD when player is OUT
    let trustedDepth = this.depthOrder;
    ...
    if (this._hadData && this._hadPlayerKey && (this.status === 'out' || this.status === 'doubtful')) {
      ...
      if (hadEntry && hadEntry.healthyAverageDepth) {
        trustedDepth = hadEntry.healthyAverageDepth;
        hadOverride = true;
        
        if (process.env.DEBUG_AVAILABILITY) {
          console.log(`🎯 HAD OVERRIDE: ${this.playerName} depth ${this.depthOrder} → ${trustedDepth} (${hadEntry.sampleSize} weeks)`);
          console.log(`   Current status: ${hadEntry.currentStatus}, HAD: ${trustedDepth}, Current depth: ${hadEntry.currentDepth}`);
        }
      }
    }
    ...
    const depthMultiplier = depthMultipliers[trustedDepth] || 0.1;
 ...
 * @param {Object} hadData - Optional: Healthy Average Depth data for override logic
 */
export function buildCanonicalAvailability(
  ...
  // V2 ENHANCEMENT: Store HAD data for use in calculateImpact
  if (hadData) {
    avail._hadData = hadData;
    avail._hadPlayerKey = `${team}_${position}_${playerName}`;
  }
