...
exports.handler = async (event) => {
  try {
    ...
    return { statusCode: 200, headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ ok: true, season: sched.season, week: parseInt(weekStr,10), games: games.length, candidates: players, meta: { using: locateScheduleJSON().replace(process.cwd(), ''), dirname: __dirname } }) };
  } catch (err) {
