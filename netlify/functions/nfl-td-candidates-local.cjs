  ...
  try {
    ...
    for (const [team,groups] of Object.entries(DEPTH)){
      ...
      for (const pos of ['RB','WR','TE']){
        if(groups[pos]) for(const pl of groups[pos].slice(0,2)){
          players.push({team,pos,player:pl.name,role:pl.role||null,td_prob:null,fair_odds:null,notes:`scaffold-inline (week ${weekStr})`});
        }
      }
    }
    return { statusCode:200, headers:{'content-type':'application/json'}, body: JSON.stringify({ok:true,season:sched.season,week:parseInt(weekStr,10),games:games.length,candidates:players}) };
  } catch (err) {
