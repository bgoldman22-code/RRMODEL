 ...
 * - NBA_PROPS_FORCE_ESPN: Set to "1" to bypass Blobs and always fetch ESPN (incident recovery)
 * - NBA_PROPS_ENABLE_CDN: Set to "0" to disable NBA CDN tier (default: enabled)
...
/**
 * Load opponent defense data for adjustments
 */
let opponentDefenseMap = null;
async function loadOpponentDefense() {
  if (opponentDefenseMap) return opponentDefenseMap;
  
  try {
    // Dynamic import of opponent defense data
    const opponentDefenseModule = await import('../../data/nba/opponent-defense/2025-26.json', { 
      assert: { type: 'json' } 
    });
    const opponentDefense = opponentDefenseModule.default;
    
    // Build lookup map with normalized team names
    opponentDefenseMap = new Map();
    for (const team of opponentDefense) {
      const tricode = normalizeTeamName(team.team);
      if (tricode) {
        opponentDefenseMap.set(tricode, {
    ...
    console.log(`✅ Loaded opponent defense for ${opponentDefenseMap.size} teams`);
    return opponentDefenseMap;
    
  } catch (err) {
    console.warn(`⚠️  Could not load opponent defense data: ${err.message}`);
 ...
 * ⚠️  DEPRECATED: Use resilient-loader.mjs instead
 * Kept for backward compatibility only
 */
async function fetchESPNBoxscores(daysBack = 25) {
  console.log(`⚠️  Using deprecated fetchESPNBoxscores() - should use resilient-loader.mjs`);
...
async function generatePrediction(stats, propType, isHome, restDays, opponentTricode) {
