...
// V2 ENHANCEMENT: HAD (Healthy Average Depth) Integration
// Identical to V1 but uses canonical-availability-v5-had.mjs with HAD support
// Stores to separate blob key 'nfl-predictions-v2-{season}-week{week}'
...
// V2: Use HAD-enhanced canonical availability
import { 
...
} from '../_lib/canonical-availability-v5-had.mjs';
...
async function generateAdvancedPredictions(games, season) {
  ...
  console.log('🔬 V2 ENHANCEMENT: HAD (Healthy Average Depth) enabled for injury depth override');
  
  let advancedMetrics = null;
  let injuries = null;
  let hadData = null; // V2: HAD lookup object
  ...
  try {
    ...
    try {
      ...
      const sampleKeys = Object.keys(hadData).slice(0, 3);
      sampleKeys.forEach(key => {
        const entry = hadData[key];
        console.log(`   ${key}: HAD=${entry.healthyAverageDepth}, current=${entry.currentDepth}, status=${entry.currentStatus}`);
      });
      
    } catch (hadError) {
      console.warn('⚠️  HAD data unavailable, V2 will use same depth logic as V1:', hadError.message);
      hadData = null; // Graceful fallback to V1 behavior
    }
...
async function saveAdvancedPredictionsToBlobs(result, season) {
  ...
  // V2: Save to separate blob storage (predictions-v2/current.json)
  ...
  await store.set("predictions-v2/current.json", JSON.stringify(blobData));
  
  console.log(`💾 V2: Saved ${rows.length} HAD-enhanced predictions to blob storage (predictions-v2/current.json)`);
  return blobData;
}
...
async function saveToPredictionsCache(result, season) {
  try {
    ...
    const cacheKey = `nfl-predictions-v2-${season}-week${week}`; // V2: separate cache key
