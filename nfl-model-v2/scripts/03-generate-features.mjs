...
/**
 * Blend team stat with league average using Bayesian prior
 * Used for early-season games where sample size is small
 */
function blendWithPriors(teamValue, leagueAvg, gamesPlayed, gamesPrior) {
  if (!config.priors?.enable) return teamValue;
  return (gamesPlayed * teamValue + gamesPrior * leagueAvg) / (gamesPlayed + gamesPrior);
}

/**
 * Calculate league averages for all metrics
 */
function calculateLeagueAverages(allTeamStats) {
  const metrics = config.feature_generation.metrics;
  const leagueAvg = {};
  
  for (const metric of metrics) {
    const values = [];
    for (const stats of Object.values(allTeamStats)) {
      if (stats && stats[metric] !== undefined) {
        values.push(stats[metric]);
      }
    }
    leagueAvg[metric] = values.length > 0 
 ...
 * Applies Bayesian priors for early-season games (< 6 games played)
 */
function calculateRollingStats(teamGames, lookbackWindow = 10, leagueAvg = {}) {
  ...
  const stats = {};
  const gamesPrior = config.priors?.games_prior || 5;
  ...
  for (const metric of config.feature_generation.metrics) {
    ...
    // Apply Bayesian prior if early season (< 6 games) and priors enabled
    if (config.priors?.enable && gamesPlayed < 6) {
      const leagueValue = leagueAvg[metric] || 0;
      rawValue = blendWithPriors(rawValue, leagueValue, gamesPlayed, gamesPrior);
    }
    
    stats[metric] = rawValue;
  }
 ...
 */
function buildGameFeatures(game, teamHistories, season, leagueAvg = {}) {
  ...
  // Calculate rolling stats (with Bayesian priors for early season)
  const homeStats = calculateRollingStats(
    homeHistory, 
    config.feature_generation.lookback_window,
    leagueAvg
  );
  const awayStats = calculateRollingStats(
    awayHistory, 
    config.feature_generation.lookback_window,
    leagueAvg
  );
  ...
  const features = {
    ...
    // Trench warfare stats (V4 placeholders for real data)
    // TODO: Replace with real PBWR/PRWR from NextGen Stats or PFF when available
    home_pass_block_win_rate: homeStats.pass_block_win_rate || 0,
    away_pass_block_win_rate: awayStats.pass_block_win_rate || 0,
    home_pass_rush_win_rate: homeStats.pass_rush_win_rate || 0,
    away_pass_rush_win_rate: awayStats.pass_rush_win_rate || 0,
    pass_block_win_rate_diff: (homeStats.pass_block_win_rate || 0) - (awayStats.pass_block_win_rate || 0),
    pass_rush_win_rate_diff: (homeStats.pass_rush_win_rate || 0) - (awayStats.pass_rush_win_rate || 0),
    
    // Pressure metrics (if available from PBP)
    pressure_diff: (homeStats.pressure_rate || 0) - (awayStats.pressure_rate || 0),
...
async function main() {
  ...
  // Calculate league averages for Bayesian priors (if enabled)
  let leagueAvg = {};
  if (config.priors?.enable) {
    console.log('\n🎯 Calculating league averages for Bayesian priors...');
    const allTeamStats = {};
    for (const team in teamHistories) {
      for (const season in teamHistories[team]) {
        const games = teamHistories[team][season];
        if (games.length > 0) {
          allTeamStats[`${team}_${season}`] = calculateRollingStats(games, 100, {});
        }
      }
    }
    leagueAvg = calculateLeagueAverages(allTeamStats);
    console.log(`   ✅ League averages calculated for ${Object.keys(leagueAvg).length} metrics`);
    console.log(`   📊 Sample: EPA offense = ${leagueAvg.epa_per_play_offense?.toFixed(3) || 'N/A'}`);
  }
  ...
  for (const season of config.seasons) {
    ...
    for (const game of allSeasonGames[season]) {
      ...
      const features = buildGameFeatures(game, teamHistories, season, leagueAvg);
