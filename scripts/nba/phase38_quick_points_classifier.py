#!/usr/bin/env python3
"""Quick sanity classifier for NBA Points props.

Purpose
-------
Train a lightweight discriminative model on existing Phase 3.7 features to see
if there's any measurable signal (AUC/ROI) for Points Over/Under outcomes.
...
POINT_MARKET_VALUES = {"player_points"}
DATE_COLUMN = "date"
...
def filter_points_market(df: pd.DataFrame) -> pd.DataFrame:
    market_col = None
    ...
    if market_col is None:
        raise KeyError("Could not find a market/stat_type column to filter Points bets.")

    mask = df[market_col].isin(POINT_MARKET_VALUES)
    if mask.sum() == 0:
        raise ValueError(
            "No Points rows found. Update POINT_MARKET_VALUES if market names changed."
        )

    df_points = df.loc[mask].copy()
    print(f"✓ Filtered to Points market via '{market_col}': {len(df_points):,} rows")
    return df_points
...
def main() -> None:
    print("=" * 80)
    print("PHASE 3.8 QUICK POINTS CLASSIFIER")
    print("=" * 80)

    df = load_training_data()
    df = filter_points_market(df)
