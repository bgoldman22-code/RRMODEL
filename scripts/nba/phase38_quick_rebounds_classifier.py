#!/usr/bin/env python3
"""Quick sanity classifier for NBA Rebounds props.

Purpose
-------
Train a lightweight discriminative model on existing Phase 3.7 features to see
if there's any measurable signal (AUC/ROI) for Rebounds Over/Under outcomes.
...
REBOUND_MARKET_VALUES = {"player_rebounds"}
DATE_COLUMN = "date"
...
def filter_rebounds_market(df: pd.DataFrame) -> pd.DataFrame:
    market_col = None
    ...
    if market_col is None:
        raise KeyError("Could not find a market/stat_type column to filter Rebounds bets.")

    mask = df[market_col].isin(REBOUND_MARKET_VALUES)
    if mask.sum() == 0:
        raise ValueError(
            "No Rebounds rows found. Update REBOUND_MARKET_VALUES if market names changed."
        )

    df_rebounds = df.loc[mask].copy()
    print(f"✓ Filtered to Rebounds market via '{market_col}': {len(df_rebounds):,} rows")
    return df_rebounds
...
def main() -> None:
    print("=" * 80)
    print("PHASE 3.8 QUICK REBOUNDS CLASSIFIER")
    print("=" * 80)

    df = load_training_data()
    df = filter_rebounds_market(df)
