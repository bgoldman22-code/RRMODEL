...
exports.handler = async function(event, context){
  try{
    ...
    const region = 'us';
    const markets = ['player_receiving_yards','player_receptions','player_passing_yards','player_passing_attempts'].join(',');
          ...
          for(const market of bk.markets || []){
            for(const outcome of market.outcomes || []){
              ...
              if(market.key === 'player_receptions'){
                ...
                lines[name].altRecFloor = Math.max(2, Math.floor((outcome.point||4)/1));
              }
    ...
    return { statusCode: 200, body: JSON.stringify({ ok:true, lines }) };
  }catch(e){
