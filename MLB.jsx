...
// --- Player-adjusted Coors multiplier ---
function coorsPlayerMultiplier(c){
  try {
    const game = String(c?.game || c?.gameId || "");
    if (!game.includes("@COL")) return 1.00;
    const seasonHR = Number(c?.seasonHR || 0);
    const seasonPA = Number(c?.seasonPA || 0);
    const hrRate = seasonPA > 0 ? (seasonHR / seasonPA) : 0;
    const maxEV = Number(c?.maxEV || c?.ev_p75 || 0);
    if (hrRate >= 0.055 || maxEV >= 112) return 1.10;
    if (hrRate >= 0.030) return (maxEV >= 108 ? 1.22 : 1.20);
    return 1.08;
  } catch {
    return 1.00;
  }
  ...
  async function build(){
    ...
    try{
      ...
      for(const c of baseCandidates){
        ...
        const parkMul = coorsPlayerMultiplier(c);
        if (typeof parkMul === "number" && isFinite(parkMul) && parkMul !== 1.00) {
          p *= parkMul;
        }
} catch {}
      ...
      for (const t of temp){
        ...
        if (c.team){
          ...
          t.parkMul = typeof parkMul==='number'? parkMul : 1.00;
if (protection_mod > 0) p = p * (1 + protection_mod);
        ...
        rows.push({
          ...
          bvp_mod, protection_mod, parkHR: (t?.parkMul ?? c.parkHR ?? null),
          why: explainRow({
