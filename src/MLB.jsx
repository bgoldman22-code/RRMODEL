...
function explainRow({ baseProb=0, hotBoost=1, calScale=1, pitcherName=null, pitcherHand=null, parkHR=null, weatherHR=null, spShare=null, c=null }){
  const reasons = [];
  const add = (text, score, positive=true) => {
    if (!text) return;
    reasons.push({ text, score: Number(score)||0, positive });
  };

  // 1) BIG H2H if meaningful
  try{
    const bvp = c?.bvp;
  ...
  // 2) Pitcher Weakness × Hitter Strength
  try{
  ...
  // Choose biggest *positive* factor if present; else biggest overall
  ...
  perGame = new Map();
    ...
    try{
      ...
      for (const t of temp){
        ...
        // === Cold-bat suppression & season baseline cap (safe, bounded) ===
        try {
          const hr7 = Number(c?.form?.hr7 ?? 0);
