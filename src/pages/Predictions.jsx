...
function Row({ row }) {
  ...
  return (
    <tr className="border-b border-gray-200">
      ...
      <td className="py-2 px-3 text-sm font-medium">{row.matchup}</td>

      <td className="py-2 px-3 text-sm">
        <div className="flex flex-col gap-1">
          <div><span className="text-gray-500">ML:</span> <span className="font-medium">{ml.team}</span> <span className="text-gray-600">({ml.price > 0 ? `+${ml.price}` : ml.price})</span></div>
          <div><span className="text-gray-500">Spread:</span> <span className="font-medium">{sp.team} {sp.line > 0 ? `+${sp.line}` : sp.line}</span></div>
          <div><span className="text-gray-500">Total:</span> <span className="font-medium">{tot.side} {tot.line}</span></div>
        </div>
      </td>
...
export default function Predictions() {
  const { rows, updated, loading, error } = usePredictions();
  const parlays = useMemo(() => buildParlays(rows||[]), [rows]);

  return (
    <div className="mx-auto max-w-6xl px-4 py-6">
      <h1 className="text-2xl font-semibold mb-1">NFL Predictions</h1>
      <p className="text-sm text-gray-500 mb-6">Updated — {updated ? new Date(updated).toLocaleString() : '—'}</p>
