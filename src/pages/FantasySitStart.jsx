...
export default function FantasySitStart() {
  ...
  const handleExportPng = async () => {
    ...
    try {
      // Get the roast content element
      const roastElement = document.getElementById('roast-content-export');
      if (!roastElement) {
        alert('Roast content not found');
        return;
      }
      ...
      header.style.cssText = 'background: white; border-radius: 20px; padding: 30px; margin-bottom: 30px; text-align: center;';
      header.innerHTML = `
        <div style="font-size: 48px; font-weight: bold; color: #991b1b; margin-bottom: 10px;">
          🔥 ${roastData.league.name}
        </div>
        <div style="font-size: 32px; color: #1e3a8a; font-weight: 600;">
          Week ${roastData.week_analyzed} Power Rankings
        </div>
        <div style="font-size: 20px; color: #6b7280; margin-top: 10px;">
          Generated by bgroundrobin.com
        </div>
      `;

      // Create content box
      const contentBox = document.createElement('div');
      contentBox.style.cssText = 'background: white; border-radius: 20px; padding: 40px; box-shadow: 0 20px 60px rgba(0,0,0,0.3);';
      
      // Clone and style the roast content
      const roastClone = roastElement.cloneNode(true);
      roastClone.style.cssText = `
        color: #1f2937;
        font-size: 18px;
        line-height: 1.8;
      `;
      ...
      // Style all headings
      const h2Elements = roastClone.querySelectorAll('h2');
      h2Elements.forEach(h2 => {
        h2.style.cssText = 'color: #991b1b; font-size: 36px; font-weight: bold; margin-top: 30px; margin-bottom: 20px; border-bottom: 3px solid #fca5a5; padding-bottom: 10px;';
      });
      
      const h3Elements = roastClone.querySelectorAll('h3');
      h3Elements.forEach(h3 => {
        h3.style.cssText = 'color: #1e3a8a; font-size: 28px; font-weight: 600; margin-top: 25px; margin-bottom: 15px;';
      });

      const pElements = roastClone.querySelectorAll('p');
      pElements.forEach(p => {
        p.style.cssText = 'color: #374151; margin-bottom: 16px; font-size: 18px;';
      });

      const strongElements = roastClone.querySelectorAll('strong');
      strongElements.forEach(strong => {
        strong.style.cssText = 'color: #991b1b; font-weight: 700;';
      });

      contentBox.appendChild(roastClone);

      // Assemble export container
      exportContainer.appendChild(header);
      exportContainer.appendChild(contentBox);
      ...
      canvas.toBlob((blob) => {
        ...
        const fileName = `${roastData.league.name.replace(/[^a-z0-9]/gi, '_')}_Week${roastData.week_analyzed}_Roast.png`;
        link.href = url;
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-4 py-2 rounded-md hover:from-purple-700 hover:to-pink-700 transition-all text-sm font-semibold shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
                  {exportingPng ? (
                    <>
                ...
                <button
                  onClick={() => {
                    const shareText = `${roastData.league.name} - Week ${roastData.week_analyzed} Power Rankings\n\nGenerated by bgroundrobin.com/fantasy-sitstart`;
