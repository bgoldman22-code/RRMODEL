...
// src/utils/why.ts (works with Vite TS or JS)
export function buildWhy(input, seed){
  ...
  if (match) push(parts, pick(T_PITCH_MATCH, rng), Object.assign({}, input, match));
  else push(parts, pick(T_PITCHER, rng), Object.assign({}, input, { pitch_list: pitchList(input.pitcher||{primary_pitches:[]}) }));
  const env = envSummary(input);
  if (env) push(parts, pick(T_ENV, rng), Object.assign({}, input, env));
  const edge_pts = (num(input.true_hr_prob)!=null && num(input.implied_prob)!=null) ? Math.round(((num(input.true_hr_prob)-num(input.implied_prob))*1000))/10 : null;
  push(parts, pick(T_MARKET, rng), Object.assign({}, input, { edge_pts }));
...
function bestPitchMatch(input){ const cand=(Array.isArray(input.hitter_vs_pitch)?input.hitter_vs_pitch:[]).find(x=>x.sample_pa>=25 && x.xiso!=null); if(!cand) return null; return { match_pitch: cand.pitch, match_xiso: cand.xiso }; }
