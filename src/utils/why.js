// src/utils/why.js
// WHY generator with optional H2H spotlight (shown only if h2h.pa >= 10)
export function buildWhy(input, seed){
  ...
  // 2) H2H spotlight (if provided and sample size is real)
  if (input?.h2h && Number(input.h2h.pa) >= 10){
    push(parts, pick(T_H2H, rng), input);
  }

  // 3) Pitch match or generic pitcher
...
const T_H2H = [
  "History vs {pitcher.name}: {h2h.pa} PA with {h2h.hr} HR ({val(h2h.iso)} ISO).",
  "{player} owns a real sample vs {pitcher.name}: {h2h.pa} PA, {h2h.hr} HR — ISO {val(h2h.iso)}."
];
