...
def fit_model_enhanced(model_name, X, y, feature_names=None):
    ...
    # FIX P0-3: Deterministic, single instantiation.
    # The previous implementation evaluated ModelClass() twice and used callable() incorrectly,
    # which can create two different model instances and is unsafe for factories/lambdas.
    ModelFactory = MODEL_REGISTRY_ENHANCED[model_name]
    model = ModelFactory()
    model.fit(X, y, feature_names)
