...
def build_strength_features(df):
    ...
    # FIX P1-4: League xG baselines must combine BOTH home_xg and away_xg.
    # Using only home_xg (for "for") and only away_xg (for "against") biases context features.
    # We want a true league scoring baseline per team-per-match side.
    ...
    for date in df['date'].unique():
        ...
        if prior_mask.sum() > 0:
            # True league mean xG per team side, computed from both columns.
            # This is the baseline for both "for" and "against" denominators.
            prior_xg = pd.concat(
                [
                    df.loc[prior_mask, 'home_xg'],
                    df.loc[prior_mask, 'away_xg'],
                ],
                ignore_index=True
            )
            league_xg_mean = prior_xg.mean()
            league_xg_for = league_xg_mean
            league_xg_against = league_xg_mean
    ...
    if df['league_avg_xg_for'].isna().any():
        overall_xg = pd.concat([df['home_xg'], df['away_xg']], ignore_index=True).mean()
        df['league_avg_xg_for'].fillna(overall_xg, inplace=True)
        df['league_avg_xg_against'].fillna(overall_xg, inplace=True)
...
def validate_temporal_integrity(df, sample_size=50, verbose=True):
    ...
    # Ensure date is datetime for all downstream comparisons.
    ...
    for idx in sample_indices:
        ...
        # FIX P1-5: Stronger leakage validation via explicit recomputation.
        # The previous check relied on "suspicious equality" coincidence.
        # We instead recompute key rolling features from PRIOR matches and assert equality.
        # This catches:
        #   - off-by-one errors (including current match)
        #   - date sorting issues
        #   - accidental inclusion of future rows
        
        # Validate home_xg_for_l5
        # IMPORTANT: home_xg_for_l5 is built from the HOME TEAM's prior matches as BOTH home+away,
        # using xG-for appropriate to venue:
        #   - when team played at home: xg_for == home_xg
        #   - when team played away: xg_for == away_xg
        if pd.notna(row.get('home_norm')) and pd.notna(row.get('home_xg_for_l5')):
            home_team = row['home_norm']
            prior_team_matches = df[
                (
            ...
            if len(prior_team_matches) > 0:
                prior_xg_for = np.where(
                    prior_team_matches['home_norm'].values == home_team,
                    prior_team_matches['home_xg'].values,
                    prior_team_matches['away_xg'].values,
                )
                prior_xg_for = pd.Series(prior_xg_for).dropna()
                if len(prior_xg_for) > 0:
                    expected = prior_xg_for.mean()
                    actual = row['home_xg_for_l5']
                    if not np.isfinite(actual) or abs(actual - expected) > 1e-6:
                        if verbose:
                            print(
                                f"   ❌ LEAKAGE/BUG: home_xg_for_l5 mismatch at idx {idx} "
                                f"(team={home_team}, date={match_date.date()}): actual={actual}, expected={expected}"
                            )
                        leakage_found = True

        # Validate away_xg_for_l5 (same logic for AWAY TEAM)
        if pd.notna(row.get('away_norm')) and pd.notna(row.get('away_xg_for_l5')):
            ...
            if len(prior_team_matches) > 0:
                prior_xg_for = np.where(
                    prior_team_matches['home_norm'].values == away_team,
                    prior_team_matches['home_xg'].values,
                    prior_team_matches['away_xg'].values,
                )
                prior_xg_for = pd.Series(prior_xg_for).dropna()
                if len(prior_xg_for) > 0:
                    expected = prior_xg_for.mean()
                    actual = row['away_xg_for_l5']
                    if not np.isfinite(actual) or abs(actual - expected) > 1e-6:
                        if verbose:
                            print(
                                f"   ❌ LEAKAGE/BUG: away_xg_for_l5 mismatch at idx {idx} "
                                f"(team={away_team}, date={match_date.date()}): actual={actual}, expected={expected}"
                            )
                        leakage_found = True
    ...
    if not leakage_found:
        ...
        # FAIL LOUD: tests must not silently pass when rolling windows are wrong.
        raise AssertionError("Temporal integrity validation FAILED: rolling feature recomputation mismatch detected")
...
def build_leakfree_features(matches_df):
    ...
    # -------------------------------------------------------------------------
    # MODEL FEATURE PHILOSOPHY (EXPLICIT - PRODUCTION SAFE)
    # -------------------------------------------------------------------------
    # Option A (FROZEN): Odds are NOT model features.
    # - btts_yes_odds / btts_no_odds are used ONLY in production_decision.py
    #   to compute fair market probabilities + edges.
    # - They MUST NOT be included in df.attrs['leakfree_feature_columns'] to
    #   prevent training/inference feature drift.
    # -------------------------------------------------------------------------

    # Optional one-time runtime log for auditability (kept minimal).
    # This prints once per feature build invocation, not per-row.
    print("🧠 Feature policy: Option A active (odds NOT model features; odds used only for edge/decisioning)")

    # Mark which columns are safe for modeling vs leaky / non-features
    feature_cols = [c for c in df.columns if c not in [
        ...
        # NOTE: Odds are NOT model features (Option A)
    ...
    # Hard guardrail: ensure odds are never accidentally included as model features
    assert 'btts_yes_odds' not in df.attrs['leakfree_feature_columns']
    assert 'btts_no_odds' not in df.attrs['leakfree_feature_columns']
